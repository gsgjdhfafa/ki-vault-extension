...
const DEFAULT_KEYWORDS = {
  // Code / Tech
  'html': 5, 'javascript': 5, 'typescript': 5, 'python': 5, 'css': 4,
  '```': 6, 'function': 4, 'async': 3, 'await': 3, 'import': 3, 'export': 3, 'class': 3,
  'docker': 5, 'dockerfile': 5, 'bash': 4, 'script': 3, 'npm': 3, 'git': 3,
  'api': 4, 'json': 3, 'webhook': 5, 'n8n': 6, 'workflow': 5, 'automation': 5,
  'vscode': 4, 'extension': 4, 'plugin': 4, 'supabase': 4, 'postgresql': 4,
  'sqlite': 4, 'indexeddb': 4, 'manifest': 3, 'raspberry': 4, 'docker-compose': 6,
  'linux': 4, 'ubuntu': 4, 'cloudflare': 4, 'nginx': 4, 'ssl': 3,
  // Business / IP
  'patent': 7, 'dpma': 8, 'marke': 6, 'schutzrecht': 7, 'geschäftsmodell': 7,
  'businessplan': 7, 'masterplan': 6, 'roadmap': 5, 'monetar': 6, 'umsatz': 5,
  'revenue': 5, 'saas': 6, 'retainer': 6, 'lizenz': 6, 'tier': 4, 'preismodell': 5,
  'ki4ki': 8, 'kunden': 4, 'akquise': 5, 'kundenwert': 5,
  // Patente — erweitert
  'epa': 6, 'wipo': 6, 'gebrauchsmuster': 7, 'prior art': 6,
  // Beschaffung / Aussonderung
  'vebeg': 7, 'ausgemustert': 6, 'aussonderung': 6, 'surplus': 5,
  'refurbished': 5, 'restposten': 5,
  // KI / Agent
  'systemprompt': 6, 'system prompt': 6, 'agent': 5, 'llm': 5, 'prompt': 4,
  'claude': 3, 'gpt': 3, 'gemini': 3,
  // TTT / Tauchen
  'ttt': 5, 'taucherteam': 6, 'triton': 5, 'tauchen': 4, 'tauchgang': 4, 'kompressor': 4,
  // Struktur-Signale
  '###': 3, '##': 2, '---': 2, 'checkliste': 4, 'infrastruktur': 5,
  'dashboard': 5, 'sot ': 5, 'source of truth': 6, 'deployment': 4, 'architektur': 4,
};

const TRASH_KEYWORDS = {
  'danke schön': -8, 'vielen dank': -8, 'super': -4,
  'kannst du mir': -5, 'erkläre mir': -5, 'was ist': -3,
  'wie heißt': -5, 'wie lautet': -5,
  'hallo': -6, 'hi ': -6, 'hey ': -6,
  'ok ': -4, 'alles klar': -5, 'perfekt': -3, 'gut gemacht': -5,
};
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (msg.type === 'EXPORT_ZIP') {
    (async () => {
      ...
      threads.forEach(t => {
        const dir = catMap[t.category] || '_KAI_VAULT/09_REFERENZ/';
